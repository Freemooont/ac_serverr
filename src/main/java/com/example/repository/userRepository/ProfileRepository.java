...
public interface ProfileRepository extends JpaRepository<Profile,Long> {
    ...
    String GET_BY_SURNAME = "SELECT * FROM cl_user u where u.last_name = :last_name";
    @Query(value = FB_ID,nativeQuery = true)
    Long verifyFbId();

    @Query(value = SELECT_USER, nativeQuery = true)
    Profile selectUserByFbId(@Param("fb_id") Long fb_id);
    ...
    @Query(value = GET_BY_SURNAME, nativeQuery = true)
    List<Profile> selectUserByLastName(@Param("last_name") String last_name);
}
