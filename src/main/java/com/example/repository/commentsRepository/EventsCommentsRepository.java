...
import static com.example.repository.commentsRepository.EventsCommentsRepository.COMMENTS_BY_STEP_WITH_USER;
import static com.example.repository.commentsRepository.EventsCommentsRepository.COMMENTS_BY_STEP;
import static com.example.repository.commentsRepository.EventsCommentsRepository.NUMBER_OF_COMMENTS;


public interface EventsCommentsRepository extends JpaRepository<EventComments,Long> {
    String COMMENTS_BY_STEP_WITH_USER = "select * from cl_events_comments t where t.feed_id = :feed_id AND t.user_id = :user_id limit :nr_of_comments offset :offset ";
    String COMMENTS_BY_STEP = "select * from cl_events_comments t where t.feed_id = :feed_id ORDER BY datetime_post DESC limit :nr_of_comments offset :offset ";
    String NUMBER_OF_COMMENTS = "select count(c.id) from cl_events_comments c where c.feed_id = :feed_id";
