...
public interface SuggestionsCommentsRepository extends JpaRepository<SuggestionComments,Long> {
    String COMMENTS_BY_STEP_WITH_USER = "select * from cl_suggestions_comments t where t.feed_id = :feed_id AND t.user_id = :user_id limit :nr_of_comments offset :offset ";
    String COMMENTS_BY_STEP = "select * from cl_suggestions_comments t where t.feed_id = :feed_id ORDER BY datetime_post DESC limit :nr_of_comments offset :offset ";
    String NUMBER_OF_COMMENTS = "select count(c.id) from cl_suggestions_comments c where c.feed_id = :feed_id";


    @Query(value = COMMENTS_BY_STEP_WITH_USER, nativeQuery = true)
    List<SuggestionComments> getSuggestionComments(@Param("nr_of_comments") int nr_of_comments,
